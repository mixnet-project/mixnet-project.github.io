...
// reverse mapping
const reverseTopoDisplayMapping = {
  'Fat-tree': 'fattree',
  'MixNet': 'mixnet',
  'Rail-optimized': 'rail-optimized',
  'TopoOpt': 'topoopt'
...
const emitUpdate = () => {
  ...
  hasChanges.value = JSON.stringify(performanceData.value) !== JSON.stringify(originalData.value)
  
  performanceData.value.forEach(row => {
    bandwidths.forEach(bw => {
      const existingDataIndex = perfData.value.findIndex(item => 
        item.type === row.type && 
...
async function loadModelData(model = selectedModel.value) {
  try {
    ...
    uniqueTopos.forEach(topo => {
      ...
      bandwidths.forEach(bw => {
        const modelData = perfData.value.find(d => 
          d.Model === model && 
          d.type === topo &&
          d.Bandwidth === bw
        )
...
async function loadPerformanceData() {
  try {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const topoTypes = headers.filter(h => !['Model', 'Bandwidth', 'type'].includes(h))
...
onMounted(async () => {
  try {
    ...
    ElMessage.error('Failed to load performance data. Please check the console for details.')
  }
})
